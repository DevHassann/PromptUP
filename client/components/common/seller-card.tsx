...
const SellerCard = ({ item, loading }: SellerCardProps) => {
  return (
    <Card className="py-4 bg-white m-3 w-full lg:w-[31%] sm:w-[46%] 2xl:w-[23%] flex flex-col items-center text-black">
      {loading ? (
        <>
        ...
        <>
          ...
          <span className={`${styles.label} py-2 text-xl text-black font-[700]`}>@{item?.name}</span>
          <div className="flex items-center">
            <span className={`${styles.label} pr-2 text-black font-[700]`}>{item?.ratings}/5</span>
            <Ratings rating={item?.ratings} />
          </div>
          <span className={`${styles.label} py-2 text-black font-[700]`}>
